$(document).ready(function(){
    ...
    // function CartLoaderScreen() {
    //     $(".cart-item-preloader").html(
    ...
    // }
     ...
     function getAuthTotalCartItem (){
        $.ajax({
            ...
            success: function (resp){
                $("#authenticated-user-total-cart-item").html(resp.data);
            },
            error : function (hrx, status, err) {
                console.log(err);
            }
        });
     }
     getAuthTotalCartItem();

     function getUnAuthTotalCartItem (){
        $.ajax({
            url: "/cart-item-count/",
            method: "GET",
            data: {
            ...
            success: function (resp){
                $("#un-authenticated-user-total-cart-item").html(resp.data);
            },
            error : function (hrx, status, err) {
                console.log(err);
            }
    ...
    $("body").on("input", '#edit-qty-input', function(){
        const product_id = $(this).attr('data-id');
        const qty = $('.edit-qty-input-'+product_id).val();

        if (qty == 0){
            qty = $('.edit-qty-input-'+product_id).val(1);
        }
        ...
        $.ajax({
            ...
            success: function(resp){
                ...
                if (resp.data == 'unauthenticated'){
                    $('#total-cart-item').html(" "+resp.total_cart_item_in_session)
                }
            },
            error: function(hrx, status, error){
                alert(status+": "+error);
